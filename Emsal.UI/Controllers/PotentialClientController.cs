    ...
    public class PotentialClientController : Controller
    {
            ...
            try
            {
                ...
                else if (iamasPerson.Name != null)
                {
                    if (iamasPerson.Adress.cityId != null)
                    {
                        ...
                        {
                            BaseOutput gaufci = srv.WS_GetPRM_AdminUnitByIamasId(baseInput, Int64.Parse(iamasPerson.Adress.cityId), true, true, true, out modelPotentialProduction.PRMAdminUnit);
                        }
                    }
                    ...
                    if (iamasPerson.Adress.districtId != null)
                    {
                        ...
                        {
                            BaseOutput gaufci = srv.WS_GetPRM_AdminUnitByIamasId(baseInput, Int64.Parse(iamasPerson.Adress.districtId), true, false, true, out modelPotentialProduction.PRMAdminUnit);
                        }
                    }
        ...
        public JsonResult GetLegalPerson(string voen)
        {
            try
            {
                ...
                {
                    VOENDATA taxesService = Emsal.WebInt.EmsalService.taxesService.getOrganisationInfobyVoen(voen);
                    //Emsal.WebInt.TaxesSRV.VOENDATA taxesService = Emsal.WebInt.EmsalService.taxesService.getOrganisationInfobyVoen("0200031191");
            ...
            try
            {
                ...
                if (model.enumCat != null)
                {
                    for (long ecv = 0; ecv < model.enumCat.Length; ecv++)
                    {
                        ...
                        {
                            BaseOutput pcb = srv.WS_GetProductionControlsByPotentialProductionId(baseInput, modelPotentialProduction.PotentialProduction.Id,true, out modelPotentialProduction.ProductionControlArray);
        ...
        public ActionResult AdminUnit(long pId = 0, long productAddressId = 0, string status = null)
        {
            try
            {
                ...
                BaseOutput bouputs = srv.WS_GetAdminUnitsByParentId(baseInput, (int)pId, true, out modelPotentialProduction.PRMAdminUnitArray);
                modelPotentialProduction.PRMAdminUnitList = modelPotentialProduction.PRMAdminUnitArray.ToList();
                ...
                if (status != null)
                {
                    if (addressDesc != "")
                    {
                        modelPotentialProduction.ProductAddress.addressDesc = addressDesc;
                    }
        ...
        public ActionResult SelectedDocuments(long PId = 0)
        {
            try
            {
                ...
                BaseOutput gpbu = srv.WS_GetProductionDocumentsByGroupId(baseInput, grup_Id, out modelPotentialProduction.ProductionDocumentArray);

                if (PId > 0)
                {
                    modelPotentialProduction.ProductionDocumentList = modelPotentialProduction.ProductionDocumentArray.Where(x => x.Potential_Production_Id == PId).ToList();
                }
                else
                {
                    modelPotentialProduction.ProductionDocumentList = modelPotentialProduction.ProductionDocumentArray.ToList();
                }
