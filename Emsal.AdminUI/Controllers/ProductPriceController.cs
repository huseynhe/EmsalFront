    ...
    public class ProductPriceController : Controller
    {
        ...
        public ActionResult Index(ProductPriceViewModel model)
        {
            try
            {
                ...
                {
                    BaseOutput envalydn = srv.WS_GetDemandProductionListNotPrice(baseInput, Convert.ToInt64(model.EnumValueYear.name), true, Convert.ToInt64(model.EnumValueRub.name), true, out model.ProductPriceDetailArray);
                }
        ...
        public ActionResult Approv(int year = 0, int rub = 0, int page = 1, string pname = null)
        {
            try
            {
                ...
                if (syear > 0 && srub > 0)
                {
                    ...
                    BaseOutput envalydn = srv.WS_GetDemandProductionListNotPrice(baseInput, Convert.ToInt64(modelproductPrice.EnumValueYear.name), true, Convert.ToInt64(modelproductPrice.EnumValueRub.name), true, out modelproductPrice.ProductPriceDetailArrayNP);

                    modelproductPrice.DNPCount = modelproductPrice.ProductPriceDetailArrayNP.ToList().Count();
                }
                else
                {
                    modelproductPrice.ProductPriceDetailList = new List<ProductPriceDetail>();
