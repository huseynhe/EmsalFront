    ...
    public class DemandProductionController : Controller
    {
        ...
        public ActionResult TotalDemandOffersGroup(int? page, long productId = -1, long userType = -1, string finVoen = null)
        {
            try
            {
                ...
                modelDemandProduction.DemandOfferProductsSearch.productId = sproductId;
                modelDemandProduction.DemandOfferProductsSearch.productIdSpecified = true;
               modelDemandProduction.DemandOfferProductsSearch.roleID = suserType;
                modelDemandProduction.DemandOfferProductsSearch.pinNumber = sfinVoen;
                modelDemandProduction.DemandOfferProductsSearch.voen = sfinVoen;
                ...
                return Request.IsAjaxRequest()
                   ? (ActionResult)PartialView("PartialTotalDemandOffersGroup", modelDemandProduction)
                   : View(modelDemandProduction);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult TotalDemandOffersGroupDetail(int? page,long productId = -1)
        {
            try
            {
                ...
                if (modelDemandProduction.DemanProductionGroupArray == null)
                {
                    modelDemandProduction.DemanProductionGroup = new DemanProductionGroup();
                }
                else
                {
                    modelDemandProduction.DemanProductionGroup = modelDemandProduction.DemanProductionGroupArray.FirstOrDefault();
                }
                modelDemandProduction.itemCount = modelDemandProduction.DemanProductionGroup.offerProductsList.Count();


                return Request.IsAjaxRequest()
                   ? (ActionResult)PartialView("PartialTotalDemandOffersGroupDetail", modelDemandProduction)
                   : View(modelDemandProduction);
            }
            catch (Exception ex)
            {
